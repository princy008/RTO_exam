...
import 'package:rto_exam/controller/practice/practice_controller.dart';
import 'package:rto_exam/generated/l10n.dart';
import 'package:rto_exam/model/question_data.dart';
import 'package:rto_exam/screens/practice/practice_card.dart';
...
class _PracticeScreenState extends State<PracticeScreen> {
  ...
  void initState() {
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) {
      practiceController.openContinueDialog(context);
    });
  }
  ...
  Widget build(BuildContext context) {
    ...
    return GetBuilder<PracticeController>(
      ...
      builder: (controller) {
        return WillPopScope(
          onWillPop: () async {
            practiceController.back(context);
            return true;
          },
          child: Scaffold(
            ...
            appBar: CommonAppBar(
              ...
              actions: [
                Obx(() {
                  final filterOptions = controller.getMenuList(context);

                  return PopupMenuButton<String>(
                    onSelected: (value) {
                      practiceController.updateFilter(value);
                    },
                    itemBuilder: (context) => filterOptions.entries
                        .map(
                    ...
                    child: Row(
                      children: [
                        CommonText(
                          text: filterOptions[practiceController.selectedFilter.value] ?? '',
            ...
            body:  controller.isLoading.value?Center(
              ...
              padding:
                  EdgeInsets.symmetric(vertical: AppDimensions.paddingMedium,horizontal: AppDimensions.paddingXSmall),
              child: Column(
                children: [
                  Expanded(
                    child: SingleChildScrollView(
                      child: Builder(builder: (context) {
                        ...
                        return PracticeCard(
                          ...
                          isBookmarked: controller.isBookmarked(item.id??0),
                          fromSigned: item.fromSign??false,
                          isOptionSign: item.isSignOption,
                          onBookmarkTap: () {
                            controller.toggleBookmark(item.id??0);
                          },
                          selectedAnswerIndex: controller.selectedAnswerIndex.value,
                          answerChecked: controller.answerChecked.value,
          ...
          Container(
            ...
            child: Row(
              ...
              children: [
                Row(
                  children: [
                    Icon(Icons.check, size: 16.r, color: AppColors.backGroundColor),
                    Spacing.width(4),
                    CommonText(
                      text: "$correctCount",
                      fontSize: AppDimensions.fontMedium,
                ...
                Row(
                  children: [
                    Icon(Icons.close, size: 16.r, color: AppColors.backGroundColor),
                    Spacing.width(4),
                    CommonText(
                      text: "$incorrectCount",
                      fontSize: AppDimensions.fontMedium,
